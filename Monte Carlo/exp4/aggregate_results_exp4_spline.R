...
#
# Usage:
#   Rscript aggregate_results_exp4_spline.R <output_dir> <n_reps> [--phase=1|2]
#
# --phase=1: Aggregate R estimator results only (run after Phase 1)
# --phase=2: Merge medflow-spline results into existing R results (run after Phase 2)
# No flag:   Aggregate everything (backward compatible)
# ============================================================

args <- commandArgs(trailingOnly = TRUE)
if (length(args) < 2) {
  stop("Usage: Rscript aggregate_results_exp4_spline.R <output_dir> <n_reps> [--phase=1|2]")
}
...
find_medflow_spline_result_files <- function(output_dir) {
  ...
  files <- unlist(lapply(candidate_dirs, function(dir_path) {
    list.files(
      dir_path,
      pattern = "^medflow_spline_rep_[0-9]+\\.csv$",
      full.names = TRUE
    )
  }), use.names = FALSE)
  ...
  file_tbl <- data.frame(
    path = files,
    rep_id = as.integer(sub("^medflow_spline_rep_([0-9]+)\\.csv$", "\\1", basename(files))),
...
if ("status" %in% names(results_df)) {
  ...
  if (n_err > 0) {
    ...
    err_file <- file.path(output_dir, "rep_errors_exp4_spline.csv")
...
# ============================================================
# Load and merge medflow-spline results (Phase 2 or full)
# ============================================================
if (phase != 1) {
  cat("\nLoading medflow-spline results...\n")

  medflow_files <- find_medflow_spline_result_files(output_dir)
  cat(sprintf("Found %d medflow-spline result files\n", length(medflow_files)))
  ...
  if (length(medflow_files) > 0) {
    ...
    if ("rep_id" %in% names(medflow_df)) {
      medflow_df <- medflow_df %>%
        dplyr::arrange(.data$rep_id) %>%
        dplyr::distinct(.data$rep_id, .keep_all = TRUE)
      cat(sprintf("Total medflow-spline replications: %d\n", nrow(medflow_df)))

      # Merge medflow-spline results into main results by rep_id
      results_df <- left_join(results_df, medflow_df, by = "rep_id")
    } else {
      cat("Warning: medflow-spline files were found but none could be read into a table with rep_id.\n")
    }
  } else {
    cat("Warning: No medflow-spline results found. Proceeding with R estimators only.\n")
  }
}
...
summary_file <- file.path(output_dir, paste0("mc_summary_exp4_spline", suffix, ".rds"))
summary_list <- list(
  true_pse = true_pse,
  true_intv = true_intv,
  pse_stats = pse_stats,
...
write.csv(pse_stats, file.path(output_dir, paste0("pse_stats_exp4_spline", suffix, ".csv")), row.names = FALSE)
write.csv(intv_stats, file.path(output_dir, paste0("intv_stats_exp4_spline", suffix, ".csv")), row.names = FALSE)
...
if (phase == 1) {
  ...
  cat("FINAL AGGREGATION COMPLETE (R + medflow-spline)\n")
} else {
  cat("AGGREGATION COMPLETE (Spline MedFlow)\n")
}
cat("=" , rep("=", 60), "\n")
