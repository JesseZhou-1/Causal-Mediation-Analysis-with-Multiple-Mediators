...
dmlmed <- function(
  ...
  # 1. Make sure the treatment is numeric:
  ...
  # Create treatment indicators for propensity models (supports count/discrete D):
  I_d <- as.numeric(data[[D]] == d)
  I_dstar <- as.numeric(data[[D]] == dstar)
  ...
  if(any(method_type %in% list(c(2),c(1,2)))){
    ...
    for(k in 1:K){
      ...
      # Fit Treatment Models (only for IPW and MR2):
      if (method %in% c("ipw", "mr2")) {
        # Estimate P(D=d|C):
        pi_d_DC <- SuperLearner(
          Y          = I_d[-predict_fold],
        ...
        # Estimate P(D=d*|C):
        pi_dstar_DC <- SuperLearner(
          Y          = I_dstar[-predict_fold],
          X          = dm_pi_DC[-predict_fold,],
          newX       = dm_pi_DC,
          family     = binomial(),
          SL.library = SL.library,
      ...
      # Fit Outcome Models (only for REG and MR2):
      if (method %in% c("reg", "mr2")) {
        # Estimate μD(M,C):
        mu_DMC <- SuperLearner(
          Y          = train_k[[Y]],
      ...
      # Compute predictions:
      if (method %in% c("ipw", "mr2")) {
        data$pi_hat_d_DC <- pi_d_DC$SL.predict
        data$pi_hat_dstar_DC <- pi_dstar_DC$SL.predict
        data$pi_hat_d_DCM <- pi_d_DMC$SL.predict
        data$pi_hat_dstar_DCM <- pi_dstar_DMC$SL.predict
      }
      if (method %in% c("reg", "mr2")) {
        data$nu_d_d <-
    ...
    # Select available columns based on method
    avail_cols <- c(Y, D)
    if (method %in% c("ipw", "mr2"))
      avail_cols <- c(avail_cols, "pi_hat_d_DC", "pi_hat_dstar_DC",
                      "pi_hat_d_DCM", "pi_hat_dstar_DCM")
    if (method %in% c("reg", "mr2"))
      avail_cols <- c(avail_cols, "mu_hat_DMC_d", "mu_hat_DMC_dstar",
                      "nu_d_dstar", "nu_d_d", "nu_dstar_d", "nu_dstar_dstar")
    stat_df2 <- main_df %>% dplyr::select(dplyr::all_of(avail_cols))
    ...
    final_calculation2 <-
      lapply(
        ...
        function(comb){
          ...
          # Compute weights (IPW and MR2 only)
          if (method %in% c("ipw", "mr2")) {
            rst_df <-
              rst_df %>%
              mutate(
          ...
          # Compute S based on method
          if (method == "ipw") {
            rst_trm_df <-
              rst_df %>%
              mutate(
                !!sym(paste0("S_",d1,"_",d2)) :=
                  !!sym(paste0("W1_", d1 ,"_", d2)) * !!sym(Y)
              ) %>%
            ...
            # MR2: full three-term influence function
            rst_trm_df <-
              rst_df %>%
              mutate(
                !!sym(paste0("S_",d1,"_",d2)) :=
    ...
    # Clean up duplicate W2 columns (IPW and MR2 only)
    if (method %in% c("ipw", "mr2")) {
      final_df2 <- final_df2 %>%
        dplyr::select(
          -.data$W2_d.x,
    ...
    final_stat2 <-
      final_df2 %>%
      summarise(
        !!paste0("ATE(", d, ",", dstar, ")_Mean") := mean(.data$ATE),
        !!paste0("NDE(", d, ",", dstar, ")_Mean") := mean(.data$NDE),
        !!paste0("NIE(", d, ",", dstar, ")_Mean") := mean(.data$NIE),
        !!paste0("ATE(", d, ",", dstar, ")_SE") := sd(.data$ATE) / sqrt(n()),
        !!paste0("NDE(", d, ",", dstar, ")_SE") := sd(.data$NDE) / sqrt(n()),
        !!paste0("NIE(", d, ",", dstar, ")_SE") := sd(.data$NIE) / sqrt(n())
      ) %>%
      pivot_longer(
  ...
  # Type 1 requires binary D with values d and dstar

  if(any(method_type %in% list(c(1),c(1,2)))){
    if (!all(unique(data[[D]][!is.na(data[[D]])]) %in% c(d, dstar))) {
      stop(paste0("Type 1 estimator requires binary treatment D with values {d, dstar}. ",
                   "Found values: ",
                   paste(sort(unique(data[[D]][!is.na(data[[D]])])), collapse = ", ")))
    }
    # Recode D to 0/1 for Type 1 propensity models
    data <- data %>% mutate(!!sym(D) := if_else(!!sym(D) == d, 1, 0))
    # Set the seed and generate the CV_folds:
    set.seed(seed)
    cf_folds <- caret::createFolds(data[,Y,drop = TRUE], K)
    # Generate the design matrices for model fitting and prediction:
