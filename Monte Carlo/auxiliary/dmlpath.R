...
dmlpath <- function(
  ...
  for(k in rev(seq_len(K))){
    ...
    if(K == 1) {
      nde_label <- paste0("NDE(", d, ",", dstar, ")")
      nie_label <- paste0("NIE(", d, ",", dstar, ")")
      ate_label <- paste0("ATE(", d, ",", dstar, ")")
      PSE[[K]] <- est[["est2"]] %>% dplyr::filter(.data$Estimand == nde_label) %>% dplyr::pull(.data$Mean)
      PSE[[K + 1]] <- est[["est2"]] %>% dplyr::filter(.data$Estimand == nie_label) %>% dplyr::pull(.data$Mean)
      SE[[K]] <- est[["est2"]] %>% dplyr::filter(.data$Estimand == nde_label) %>% dplyr::pull(SE)
      SE[[K + 1]] <- est[["est2"]] %>% dplyr::filter(.data$Estimand == nie_label) %>% dplyr::pull(SE)
      ATE <- est[["est2"]] %>% dplyr::filter(.data$Estimand == ate_label)
      ...
      ATE <- est[["est2"]] %>% dplyr::filter(.data$Estimand == paste0("ATE(", d, ",", dstar, ")"))
