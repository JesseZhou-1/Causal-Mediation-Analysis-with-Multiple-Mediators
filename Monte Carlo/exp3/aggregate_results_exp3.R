#!/usr/bin/env Rscript
# ============================================================
# Aggregate Monte Carlo Results for Experiment 3
...
if (length(args) < 2) {
  stop("Usage: Rscript aggregate_results_exp3.R <output_dir> <n_reps>")
}
...
cat("=" , rep("=", 60), "\n", sep = "")
cat("Aggregating Experiment 3 Monte Carlo Results\n")
cat("=" , rep("=", 60), "\n\n")
...
exp3_params <- list(
  D_alpha0 = 0.7, D_alpha1 = 0.3,
  M1_beta0 = -1.5, M1_beta_D = 0.5, M1_beta_C = 0.4,
  M2_gamma_D = 0.4, M2_gamma_M1 = 0.6, M2_gamma_C = 0.3, M2_sigma = 1.0,
  Y_delta_D = 0.3, Y_delta_M1 = 0.4, Y_delta_M2 = 0.5, Y_delta_C = 0.3,
  Y_cuts = c(-0.5, 0.8)
)

compute_true_pse <- function(N_truth = 1000000L, par = exp3_params, d = 5, dstar = 0) {
  set.seed(99999)
  C <- rnorm(N_truth)
...
compute_true_intv <- function(N_truth = 1000000L, par = exp3_params, d = 5, dstar = 0) {
  set.seed(99999)
  C <- rnorm(N_truth)
...
# (No IPW interventional for exp3)
# ============================================================
cat("\n")
cat("=" , rep("=", 60), "\n", sep = "")
...
# ============================================================
summary_file <- file.path(output_dir, "mc_summary_exp3.rds")
summary_list <- list(
  true_pse = true_pse,
  true_intv = true_intv,
  pse_stats = pse_stats,
...
write.csv(pse_stats, file.path(output_dir, "pse_stats_exp3.csv"), row.names = FALSE)
write.csv(intv_stats, file.path(output_dir, "intv_stats_exp3.csv"), row.names = FALSE)
